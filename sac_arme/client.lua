...
local dbtxterr = "Je ne peux sortir cette arme que d'une voiture, d'un sac à dos ou d'un sac tactique"

local bwtxterr = "Cette arme ne peut être retirée que d'une voiture."
				...
				if isWeaponBig(weapon) then
					...
					if GetVehiclePedIsIn(playerPed, false) == 0 and DoesEntityExist(vehicle) and IsEntityAVehicle(vehicle) and bigWeaponOut == false then
						...
						if duffle == true then
							...
							if bagIndex == 40 or bagIndex == 41 or bagIndex == 44 or bagIndex == 45 or accessIndex == 2 or accessIndex == 12 or (accessIndex == 4 and GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_f_freemode_01")) then 
...
function drawNotification(title, subject, msg)
